...
write.csv(resultats,"C:\\Users\\maurinau\\Documents\\AntibioticShortage\\Sensitivity_Analysis\\ResultsRCCN500_noabs.csv")



#Ouvrir fichier Resultats
resultats<-read.csv("C:\\Users\\maurinau\\Documents\\AntibioticShortage\\Sensitivity_Analysis\\ResultsRCCN500_noabs.csv")
...
creerDatframeSPRCC<-function(outcome,scenario){
  dataframe_name <- sprintf("out%sS%d", outcome, scenario)
  res<-pcc(X = resultats_lhs,rank = TRUE,semi=TRUE,y = get(dataframe_name), nboot=20)
  data1<-data.frame(Scenario=numeric(),Outcome=character(),Variable=character(),Moyenne=numeric(), Ecart_type=numeric() )
  data1[1,]<-c(scenario,outcome,"Rcr",res$SPRCC$original[1],res$SPRCC$`std. error`[1])
  data1[2,]<-c(scenario,outcome,"Ramox",res$SPRCC$original[2],res$SPRCC$`std. error`[2])
  data1[3,]<-c(scenario,outcome,"Rmacro",res$SPRCC$original[3],res$SPRCC$`std. error`[3])
  data1[4,]<-c(scenario,outcome,"ConsoAmox",res$SPRCC$original[4],res$SPRCC$`std. error`[4])
  data1[5,]<-c(scenario,outcome,"ConsoMacro",res$SPRCC$original[5],res$SPRCC$`std. error`[5])
  data1[6,]<-c(scenario,outcome,"pi",res$SPRCC$original[6],res$SPRCC$`std. error`[6])
  data1[7,]<-c(scenario,outcome,"LambdaAge",res$SPRCC$original[7],res$SPRCC$`std. error`[7])
  data1[8,]<-c(scenario,outcome,"TreatmentDuration_Amox",res$SPRCC$original[8],res$SPRCC$`std. error`[8])
  data1[9,]<-c(scenario,outcome,"TreatmentDuration_Macro",res$SPRCC$original[9],res$SPRCC$`std. error`[9])
  return(data1)
}

creerDatframePRCC<-function(outcome,scenario){
  dataframe_name <- sprintf("out%sS%d", outcome, scenario)
...
for(i in ListeOutcome<-c("Ig","PropAns","PropMr","PropAnsMr")){
  for(j in 1:4){
    dfout<-creerDatframePRCC(i,j)
    dfRCC<-rbind(dfRCC,dfout)
  }
}

write.csv(dfRCC,"C:\\Users\\maurinau\\Documents\\AntibioticShortage\\Sensitivity_Analysis\\DataframeSPRCCN500_noabs.csv")
...
```{r lire et combiner RDS files plot heatmap}

#Ouvrir fichier Resultats

resultats_PartialRankCorrelationCoefficients<-read.csv("C:\\Users\\Aurélie\\Documents\\AntibioticShortage\\Sensitivity_Analysis\\DataframePRCCN500.csv")

#resultats_SemiPartialRankCorrelationCoefficients<-read.csv("C:\\Users\\maurinau\\Documents\\AntibioticShortage\\Sensitivity_Analysis\\DataframeSPRCCN500.csv")
...
png("C:\\Users\\Aurélie\\Documents\\AntibioticShortage\\Figures\\Figure4_HeatMapPRCC.png", width = 1000, height = 600)


ggplot(df, aes(x = Variable, y = Scenario, fill = Moyenne)) +
  geom_tile() +
