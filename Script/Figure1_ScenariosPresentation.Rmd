...
LancementEffetScenario <- function(z,time){
   ...
   for (k in 1:4){
    ...
    DDJAmox=c(1.5,1.5,1.5,1.5)
    DDJMacro=c(1.1,1.1,1.1,1.1)
    ...
    ReductionFreq=c(1,1,1,1)
    AugmentationFreq=c(0,0,0,0)
    DDJAmox=c(1.5,1.5*0.75,1.5*0.5,1.5*0.25)
    DDJMacro=c(1.1,1.1,1.1,1.1)
    ...
    for(j in 1:4){
      ...
      out_odin = out_odin %>%
        mutate(
          ...
          `Amoxicilline_DDJ`=(DDJAmox[j]*`Amox[1]`*1000)/(70000000*0.055),
          `Amoxicilline_Inst`=(`AmoxInst[1]`*1000)/(70000000*0.055),
          `Macrolide_Phi`=`Macro[1]`*1000/(70000000*0.055),
          `Macrolide_DDJ`=(DDJMacro[j]*`Macro[1]`*1000)/(70000000*0.055),
...
p<-ggplot(out_long,aes(x=t,y=count,colour=Antibiotique,linetype=as.factor(Compartment)))+
  ...
  scale_color_manual(values=c(Amoxicilline="orange",Macrolide="purple" ),name = "Antibiotic", labels = c("Amoxicillin", "Macrolide"))+
  xlab("Time (days)")+ylab("")+ggtitle("Antibiotic exposure")+
...
# labeller_fig1=labeller(Scenario=c("1"="Dose/days reduction (S1)","2"="Treatment duration reduction (S2)","3"="Exposure rate reduction (S3)","4"="Alternative treatment (S4)"), Antibiotique=c("Pen"="Penicillin susceptibility","Macro"="Macrolide susceptibility","Multi"="Multi-resistant strains"))

labeller_fig1=labeller(Scenario=c("1"="S1","2"="S2","3"="S3","4"="S4"), Antibiotique=c("Pen"="Penicillin ","Macro"="Macrolide ","Multi"="Multidrug"))

# Ouvrir un périphérique graphique pour sauvegarder le plot en tant qu'image PNG
png("C:\\Users\\maurinau\\Documents\\AntibioticShortage\\Figures\\FigureS1.png", width = 1200, height = 1200)

ggplot(out_long,aes(x=t,y=count,colour=Compartment,linetype = as.factor(Rcr)))+
  geom_rect(aes(xmin =100, xmax = Inf, ymin = -Inf, ymax = Inf),fill = "#FFE4E1", alpha = 0.04,inherit.aes = FALSE)+
