---
title: "Shortage Management Strategies Impact "
...
p <- ggplot(out_long, aes(x = t, y = count, colour = Antibiotique, linetype = as.factor(Compartment))) +
  geom_rect(aes(xmin = 100, xmax = Inf, ymin = -Inf, ymax = Inf),
            fill = "#FFE4E1", alpha = 0.04, inherit.aes = FALSE) +
  geom_line(linewidth = 0.5) +
  scale_linetype_manual(
    name = "Outcome",
    values = c("Phi" = "solid", "DDJ" = "dotted", "Inst" = "dashed"),
    labels = c("Phi" = "Exposed prevalence/1,000 children",
               "DDJ" = "Number of DDD/1,000 children/d",
               "Inst" = "Exposed incidence/1,000 children/d")
  ) +
  scale_color_manual(
    values = c(Amoxicilline = "orange", Macrolide = "purple"),
    name = "Antibiotic",
    labels = c("Beta-lactam", "Macrolide")
  ) +
  xlab("Time (days)") +
  ggtitle("Antibiotic exposure") +
  ...
  theme(
    ...
    plot.title = element_text(size = 8, face = "bold", hjust = 0.5, margin = margin(b = -5)),
...
# pdf(file = here::here("Figures", "Figure1a.pdf"), width = 4/2.54, height = 12/2.54)
# p
# dev.off()

emf(file = here::here("Figures", "Figure1a.emf"), width = 4/2.54, height = 12/2.54)
p
dev.off()

emf(file = here::here("Figures", "Figure1alegend.emf"), width = 18/2.54, height = 12/2.54)
p
dev.off()
...
p<-ggplot(summary_stats, aes(x = Compartment, y = mean_value, fill = Compartment, alpha = factor(t))) +
  ...
  scale_fill_manual(
    ...
    values = c("S" = "#009E73",  # Vert pour S
               "I" = "#F0E442",  # Jaune pour I
               "R" = "#D55E00"),  # Rouge pour R
  ...
  geom_errorbar(data = dplyr::filter(summary_stats,t==0), aes(ymin=min, ymax=max), position=position_nudge(x=-0.1),width = 0.2, linewidth = 0.2)+
  geom_errorbar(data = dplyr::filter(summary_stats,t==365), aes(ymin=min, ymax=max), position=position_nudge(x=0.25),width = 0.2, linewidth = 0.2)+
  geom_text(aes(label=round(mean_value,2)),vjust=-0.8,position = position_dodge(0.9),show.legend = FALSE,size=2,face = "bold")+
  xlab("Sensitivity level in colonized")+ylab("Proportion")+
  facet_rep_grid(Scenario~Antibiotique, labeller = labeller_fig1,scales="free",repeat.tick.labels = FALSE)+
  theme_clean()+
  theme(legend.background = element_rect(fill = "white", colour = "black", linewidth = 0.2),
...
# png(file = here::here("Figures", "Figure1b.png"), width = 1200, height = 1200)
# p
# dev.off()

# pdf(file = here::here("Figures", "Figure1b.pdf"), width = 12/2.54, height = 12/2.54)
# p
# dev.off()

emf(file = here::here("Figures", "Figure1b.emf"), width = 10/2.54, height = 12/2.54)
p
dev.off()
...
```{r values of the changes observed in Figure 1 with CI }
...
p<-ggplot(out_long,aes(x=t,y=count,colour=Compartment,linetype = as.factor(Rcr)))+
  ...
  scale_color_manual(values=c(R="#D55E00",S="#009E73",I="#F0E442" ),name = "Antibiotic sensitivity level", labels = c("Susceptible", "Intermediate", "Resistant"))+
...
pdf(file = here::here("Figures", "FigureS1VariationRcr.pdf"), width = 1200/81, height = 1200/81)
p
dev.off()
