...
OptimVi <- function(x,y) {
    mod <- gen$new( US_0=c((70000000*(1-(0.0163+0.0016)))*(1-pi)*0.055),
               ...
               fA1=1 ,fA2=y[1] ,fA3=y[2] ,fA4=y[3] ,fA5=y[4],fA6=y[5],fA7=y[6],fA8=y[7],
               #fA1=1, fA2=0.9978194, fA3=0.9734585, fA4=0.8477300, fA5=0.7918005, fA6=0.7917973, fA7=0.7917973, fA8=0.7917973,
               fMS=1,fMR=y[8],
               #fMS=1,fMR=0.9938748 ,
...
x0=c(0.01)
y=c(0.9999748 ,0.9916096 ,0.9420954 ,0.9192299 ,0.9192278 ,0.9192375 ,0.9194097,0.9938509)


#y=c(0.9838688, 0.9730135, 0.9705434, 0.9524315, 0.9460251 ,0.9460251, 0.9448434, 0.9455012, 0.9849666, 0.9819427)
optim(par=x0,OptimVi,y=y,control = list(maxit = 20000))
# 0.016694460 0.006679718 0.006710317 0.004614193


x=seq(0.05,0.06,0.001)
y<-sapply(x,OptimVi,y=y)
...
x0=c(0.99,0.98,0.93,0.9,0.9,0.8,0.8,0.95)
y=c(0.05464062)
optim(x0,OptimFi,y=y,method="L-BFGS-B",lower=c(0.99,0.98,0.93,0.9,0.9,0.8,0.8,0.95), upper=c(1,1,1,1,1,1,1,1))
...
for(i in 0.054:0.055){
  x0=c(0.99,0.98,0.93,0.9,0.9,0.8,0.8,0.95)
  y=c(i)
  optim(x0,OptimFi,y=y,method="L-BFGS-B",lower=c(0.99,0.98,0.93,0.9,0.9,0.8,0.8,0.95))
}
...
```{r optim Vi et Fi automatiquement}

vinit=c(0.05)

for(i in 1:1){
  print(i)
  x0_fi=c(0.99,0.9,0.8,0.8,0.8,0.8,0.8,0.95)
  Optim_fi=optim(x0_fi,OptimFi,y=vinit,method="L-BFGS-B",lower=c(0.99,0.9,0.8,0.8,0.8,0.8,0.8,0.95), upper=c(1,1,1,1,1,1,1,1,1))
  finit=Optim_fi$par
  cat("étape 1 finie, les valeurs trouvées sont :", finit[1],"",finit[2],"",finit[3],"",finit[4],"",finit[5],"",finit[6],"",finit[7],"",finit[8]," $value = ",Optim_fi$value )
  x0_Vi=c(0.01)
  Optim_vi=optim(x0_Vi,OptimVi,y=finit,control = list(maxit = 20000))
  vinit=Optim_vi$par
  cat("étape 2 finie, les valeurs trouvées sont :", vinit[1]," $value = ",Optim_vi$value )
  Optim_fi=optim(x0_fi,OptimFi,y=vinit,method="L-BFGS-B",lower=c(0.99,0.9,0.8,0.8,0.8,0.8,0.8,0.95), upper=c(1,1,1,1,1,1,1,1,1))
  finit=Optim_fi$par
  cat(vinit[1],", ", finit[1],"",finit[2],"",finit[3],"",finit[4],"",finit[5],"",finit[6],"",finit[7],"",finit[8])
}

#2017 datas
#0.05529687 ,  0.9995909  0.9909829  0.9382162  0.9138594  0.9138738  0.9138639  0.9141716  0.9920134

#2019 datas
#0.0549375 ,  0.9994428  0.9912299  0.9403102  0.9167525  0.9167867  0.9168219  0.8  0.993451

#2021 datas
#0.05473437 ,  0.9997898  0.9914586  0.9415906  0.9185046  0.9184981  0.91849  0.9186313  0.9938367
